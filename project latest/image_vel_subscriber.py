...
current_pose_data = np.load('current_pose.npy')
current_pose_data_per_episode = np.load('current_pose_per_episode.npy')
episode_length = np.load("episode_length.npy")
...
def get_current_pose(current):
    ...
    pose_current = [float(data_as_list[10][9:]),float(data_as_list[11][9:]),float(data_as_list[12][9:]),
                    float(data_as_list[14][9:]),float(data_as_list[15][9:]),float(data_as_list[16][9:]),float(data_as_list[17][9:])]
...
def callback(image,vel,current):
    ...
    current_pose_data = np.concatenate((current_pose_data,np.reshape(current_pose,[1,1,7])))
...
if inp =="y":
    ...
    np.save("velocity_data_per_episode",velocity_data_per_episode)
    ...
    np.save('current_pose_per_episode',current_pose_data_per_episode)
