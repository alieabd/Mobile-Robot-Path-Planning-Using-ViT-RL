...
def Double_Q_network():
 ...
 #   x_vel=Dense(64, activation="relu")(x_2)
    ...
    Q_1.compile(optimizer=Adam(learning_rate = alpha),loss = 'mae') 
...
###################################################################################################    
    ...
    Q_2.compile(optimizer=Adam(learning_rate = alpha), loss = 'mae')
...
class SoftActorCritic:
    ...
    def update_D_Qn(self,n_state,action,limit,G_reward,C_reward,traj_reward):
        n_st_value = self.target_v_net(n_state)

        r = self.reward_net(n_state)
        
        if abs(tf.reduce_mean(r)) <= limit:
            
                target_Q = traj_reward + G_reward + C_reward + 15*r + n_st_value
        else:
                target_Q = traj_reward + G_reward + C_reward + r + n_st_value
        
        #print('training target Q networks ')
    ...
    def update_value_net(self,st,act,batch_size):
            ...
            act_th = abs(act_th) + 0.00001
            act_vel = abs(act_vel) + 0.00001
    ...
    def memory_store(self,state, action, next_state, goal,proccessed_state,proccessed_n_state,G_reward,C_reward,traj_reward):
    
        self.memory.append((state, action, next_state, goal,proccessed_state,proccessed_n_state,G_reward,C_reward,traj_reward))
        return self.memory
    ...
    def train_Actor(self,state,goal,batch_size):
        ...
        with tf.GradientTape() as grad:
            ...
            action_loss= -tf.reduce_mean(Q - tf.math.log(abs(act_th)+0.0001) - tf.math.log(abs(act_vel)+0.0001))
            reward_loss = -tf.reduce_mean(0.0*reward + Q - tf.math.log(abs(act_vel)+0.00001)- tf.math.log(abs(act_th)+0.00001))         
...
def train(memory):
    ...
    sac.update_D_Qn(pro_n_state,act,0.01,G_reward,C_reward,traj_reward)
            ...
            for t in range(T):
                ...
                if done_1 == True:
                       # current_pose = get_current_pose()
                        ...
                        sac.memory_store(state,action,n_state,goal,proccessed_state,proccessed_n_state,G_reward,C_reward,traj_reward) 

                        memory = sac.memory
                        train(memory)
                        print('episode has ended : We fucked an obstacle , Kir to hossein')
                        print(f"total reward at episode {episode + 1} is {C_reward + Return}")
                        break
                        ...
                        #state = image_data   ################## shulud be placed in the first loop 
